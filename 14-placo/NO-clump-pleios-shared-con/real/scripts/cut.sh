...
# for CODE, select shared concordant for clumping
TARGET_LS=$( ls ../2-compare-m-f/real/outputs/*/shared_pleios_gw_${TYPE}.txt | grep ${CODE} )


TARGET_SNPS_FILE=./real/tmp/${CODE}.${TYPE}

cat ${TARGET_LS}  | cut -f1 | grep -v -w SNP  > ${TARGET_SNPS_FILE}
...
cat $FILE | grep -w -f ${TARGET_SNPS_FILE}  | cut -f1,2,3,4,5,6,15,16,18  >> ${OUTDIR}/${CODE} 
