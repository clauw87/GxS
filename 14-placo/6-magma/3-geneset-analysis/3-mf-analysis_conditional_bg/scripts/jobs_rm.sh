#!/bin/bash
#
#SBATCH --partition=haswell # partition (queue)
...
then
  ...
  #BD=$(cat ${JOBLIST} | sed -n ${SLURM_ARRAY_TASK_ID}p | cut -d" " -f1)
...
MAGMA_FOLDER=../../../../../tmp_claudia/magma/

# ../../2-gene-analysis/2-gene-analysis/real/outputs
...
GENESETS_FILE=./real/inputs/msigdb.v2023.2.Hs.entrez.gmt
BACKGROUND_FILE=./real/inputs/msigdb_background.gmt


cat ${GENESETS_FILE}  > ./real/tmp/test.sets
cat ${BACKGROUND_FILE} >> ./real/tmp/test.sets


SETS_FILE=./real/tmp/test.sets

OUT=./real/outputs/${CODE}.geneset.analysis
