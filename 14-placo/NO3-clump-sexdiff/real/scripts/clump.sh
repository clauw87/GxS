...
#zcat ${INPUT_FILE} | cut -f1,2,3,4,5,26 > 

# Variables
#CODE=$(basename ${INPUT_FILE} | cut -d '.' -f1)
#CODE=$(echo ${INPUT_FILE} | cut -d '/' -f11)
CODE=$(echo ${INPUT_FILE} | cut -d'/' -f4 | cut -d '.' -f1)
...
do
echo ${CHR}
python real/scripts/clump.py ${INPUT_FILE} ${CODE} ${CHR} ${OUTPUT_DIR_CODE}
#${CLUMPFIELD}
done
