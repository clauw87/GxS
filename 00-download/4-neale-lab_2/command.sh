...
# exit
  ...
  cat ${INFO} |   grep -wf real/inputs/target.list  | cut -f1,6  | grep -v v2 > ./real/inputs/info_cut
 
  INFO_CUT=./real/inputs/info_cut
  ...
  cat ./real/inputs/info_cut  > ./real/inputs/jobsList.txt
  ...
  #fjob=$(sbatch --array=1-${JOBS_COUNT} ${COMMAND_F})
  #mjob=$(sbatch --array=1-${JOBS_COUNT} ${COMMAND_M})

  # after both jobs are finished 
 # COMMAND_REFOR=" \
 #    ./real/scripts/command_reformat.sh \
 #        "
  #srun --dependency=afterany:$fjob:$mjob ${COMMAND_REFOR}
