...
then
  ...
  FILE_F=$(cat ./real/inputs/jobsList.txt | grep -w female | sed -n 1p | cut -f2 | cut -d " " -f2)
  ...
  FILE_F=$(cat ./real/inputs/jobsList.txt | grep -w female | sed -n ${SLURM_ARRAY_TASK_ID}p | cut  -f2 | cut -d " " -f2)
...
wget -O ./real/outputs/${CODE}_f.gz ${FILE_F}

#wget -O	./real/outputs/${CODE}_m ${FILE_M}.gz
