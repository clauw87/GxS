...
mpaircode=$(echo ${mpair} | cut -d'/'  -f5)
fpaircode=$(echo ${fpair} | cut -d'/'  -f5)

cat ${mpair} | cut -d ',' -f2 | tail -n +2 > ./real/tmp/${mpaircode}_m_snps.txt
cat ${fpair} | cut -d ',' -f2 | tail -n +2 > ./real/tmp/${fpaircode}_f_snps.txt
sort  ./real/tmp/${mpaircode}_m_snps.txt > ./real/tmp/${mpaircode}_m_sorted 
sort  ./real/tmp/${fpaircode}_f_snps.txt > ./real/tmp/${fpaircode}_f_sorted
comm -12 ./real/tmp/${mpaircode}_m_sorted ./real/tmp/${fpaircode}_f_sorted > ./real/tmp/${mpaircode}:${fpaircode}_common_snps.txt # 419
...
Rscript ./real/scripts/fmcompare.R $mpaircode $fpaircode $mpair $fpair
